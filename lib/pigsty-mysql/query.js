...
ConnectionPool.prototype.release = function(connection) {
  ...
  if (connection) {
    ...
    if (!connection.bad) self.free[id] = connection;
    delete self.used[id];
  }
...
ConnectionPool.prototype.get = function(callback, force) {
  var self = this;
  ...
  if (self.total() >= self.max_size && !force) {
...
//    callback(null, connection);
  ...
  connection.exec = function(query, parameters) {
    ...
    if (connection.bad) {
      debug('connection is bad;', connection._pool_id, "will reconnect and reattempt to query.");
      return null;
    } else {
  ...
  connection.on('error', function(data) {
    console.error("warning: connection failed, setting connection to bad.", connection._pool_id, "msg:", data, ". Connection will be respawned.")
    debug('failed query:', connection._active_query);
    connection.bad = true;
...
function Query(connection, query, force) {
...
Query.prototype._connection = function(callback) {
  ...
  if (self.container.connection.bad) {
    ...
    debug('connection is bad:', self.container.connection._pool_id, ' getting new connection');
    self.container.pool.release(self.container.connection);
...
Query.prototype.retryable = function(work, callback) {
  ...
  var run = function() {
    ...
    if (attempts > 0) debug('retrying query:', self.query);
    ...
    work(function(err, result) {
      ...
      if (err) {
        if (attempts > 5) {
          ...
          debug("Unable to execute:" , self.query, ", attempts:", attempts);
...
var fn = function(options, callback) {
  ...
  pool.get(function(err, connection) {
    ...
    var result = function(query) {
      return new Query(container, query, options.force);
    };
