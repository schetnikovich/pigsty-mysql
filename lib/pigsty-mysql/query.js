...
ConnectionPool.prototype._connection = function(id, callback) {
  ...
  connection.test = function(callback) {
    ...
    q.on('error', function(err) {
      console.error("[X] Connection Error:", connection._pool_id, err);
      connection.bad = true;
      return callback(err); 
    });
...
//    callback(null, connection);
  ...
  connection.on('error', function(data) {
    console.error("connection error: ", connection._pool_id, data, connection.bad, connection._active_query);
    connection.bad = true;
...
Query.prototype._connection = function(callback) {
  var self = this;
  // console.log("XXX CONNECITON FETCH for ", self.query);
  if (self.container.connection.bad) {
    ...
    self.container.pool.get(function(err, connection) {
      // console.log("XXX POOL GET RETURNED", err, self.query)
...
Query.prototype.retryable = function(work, callback) {
  ...
  var run = function() {
    ...
    work(function(err, result) {
      ...
      if (err) {
        if (attempts > 5) {
          console.error("Failure executing:" , self.query, "too many attempts:", attempts);
          return callback(err);
        } else {
          console.error("Unable to execute:" , self.query, "retrying:", attempts);
